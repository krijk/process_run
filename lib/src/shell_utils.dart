...
import '/src/shell_environment.dart';
import '../cmd_run.dart';
import '../shell.dart';
import 'bin/shell/import.dart';
import 'common/constant.dart';
import 'env_utils.dart';
import 'io/shell_words.dart' as io show shellSplit;
...
List<String?> scriptToCommands(String script) {
  ...
  for (String line in LineSplitter.split(script)) {
    line = line.trim();
...
String get userAppDataPath => _userAppDataPath ??= () {
      final String? override = platformEnvironment[userAppDataPathEnvKey];
      if (override != null) {
        return override;
      }
...
Map<String, String> environmentFilterOutVmOptions(
  ...
  final String? vmOptions = platformEnvironment['DART_VM_OPTIONS'];
  if (vmOptions != null) {
    environment = Map<String, String>.from(platformEnvironment);
    environment.remove('DART_VM_OPTIONS');
  }
  final String? tekartikVmOptions = platformEnvironment['TEKARTIK_DART_VM_OPTIONS'];
...
String shellJoin(List<String> parts) =>
    parts.map((String part) => shellArgument(part)).join(' ');

/// Find command in path
String? findExecutableSync(String command, List<String?> paths) {
  for (final String? path in paths) {
    final String commandPath = absolute(normalize(join(path!, command)));

    if (Platform.isWindows) {
      for (final String ext in windowsPathExts) {
        final String commandPathWithExt = '$commandPath$ext';
      ...
      final FileStat stats = File(commandPath).statSync();
