...
class ShellIo implements Shell {
  ...
  bool kill([ProcessSignal signal = ProcessSignal.sigterm]) {
    // const io.ProcessSignal ioProcessSignal = io.ProcessSignal.sigterm;
    return impl.kill();
  }
  ...
  Future<List<ProcessResult>> run(String script,
      ...
      _wrapIoException(() async {
        final List<io.ProcessResult> ioResult = await impl.run(script,
            onProcess: onProcess == null ? null : (io.Process process) {},);
        return ioResult
            .map((io.ProcessResult ioProcessResult) => ProcessResultIo(ioProcessResult))
            .toList();
      });
...
class ShellExceptionIo implements ShellException {
  ...
  ProcessResult? get result {
    final io.ProcessResult? implResult = impl.result;
    if (implResult != null) {
