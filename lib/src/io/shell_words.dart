...
List<String> shellSplit(String command) {
  final StringScanner scanner = StringScanner(command);
  ...
  while (!scanner.isDone) {
    final int next = scanner.readChar();
    switch (next) {
      case $backslash:
      ...
      case $single_quote:
        ...
        final int firstQuote = scanner.position - 1;
...
void _checkUnmatchedQuote(StringScanner scanner, int openingQuote) {
  ...
  final String type = scanner.substring(openingQuote, openingQuote + 1) == '"'
      ? 'double'
      : 'single';
