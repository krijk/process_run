...
import '/process_run.dart' hide run;
...
abstract class FlutterBinInfo {
  ...
  static FlutterBinInfo? parseVersionOutput(String resultOutput) {
    Version? version;
    String? channel;
    final Iterable<String> output = LineSplitter.split(resultOutput)
        .join(' ')
        .split(' ')
...
Future<FlutterBinInfo?> _getFlutterBinInfo() async {
  ...
  // Tools • Dart 2.4.0
  try {
    final List<ProcessResult> results = await run('flutter --version', verbose: false);
