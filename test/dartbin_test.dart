...
void defineTests() {
  group('dartbin', () {
    group('dart', () {
      ...
      test('run', () async {
        final ProcessResult result = await Process.run(dartExecutable!, ['--version']);
      ...
      test('dart_empty_param', () async {
        final ProcessResult result = await Process.run(dartExecutable!, []);
    ...
    test('which', () {
      final String? whichDart = whichSync('dart');
    ...
    group('help', () {
      test('dart', () async {
        ...
        expect(result.exitCode, 0);
        Version minVersion = Version(2, 10, 0, pre: '1');
        String reason = 'Version output now on stdout since $minVersion';
