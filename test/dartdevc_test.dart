...
void defineTests() {
  group('dartdevc', () {
    test('help', () async {
      final ProcessResult result = await runCmd(DartDevcCmd(['--help']));
      expect(result.stdout, contains('Usage: dartdevc'));
      expect(result.exitCode, 0);
    });
    test('version', () async {
      final ProcessResult result = await runCmd(DartDevcCmd(['--version']));
    ...
    test('build', () async {
      ...
      final String destination = join(testDir, 'dartdevc_build', 'main.js');
