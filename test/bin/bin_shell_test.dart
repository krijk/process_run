...
String safeLocalEnvFile = '.dart_tool/process_run/test/test_local_env3_safe.yaml';

ShellEnvironment safeShellEnvironment = ShellEnvironment()
...
void main() {
  group('bin_shell', () {
    test('version', () async {
      final String output = (await shell.run('ds --version')).outText.trim();
      await shell.run('ds env edit -h');
      expect(Version.parse(output), shellBinVersion);
    });

    test('help', () async {
      final Iterable<String> outLines = (await shell.run('ds --help')).outLines;
    ...
    group('env', () {
      ...
      test('delete', () async {
        ...
        try {
          ...
          final File file = File(safeLocalEnvFile);
          if (file.existsSync()) {
      ...
      group('path', () {
        ...
        test('set/get/delete', () async {
          const String uniqueName = 'e1ccfaef-d320-4af6-a157-6f006dd7b6e0';
          await shell.run('ds env path prepend $uniqueName');
          String text = (await shell.run('ds env path get $uniqueName')).outText;
      ...
      group('var', () {
        test('var set/get/delete', () async {
          ...
          String text = (await safeShell.run('ds env var get TEST_VALUE')).outText;
      ...
      group('alias', () {
        ...
        test('set/get/delete', () async {
          ...
          String text = (await shell.run('ds env alias get TEST_ALIAS')).outText;
          expect(text, contains('dummy1'));
    ...
    group('file_content', () {
      test('addAlias', () async {
        final EnvFileContent fileContent = EnvFileContent('dummy')..lines = [];
