...
void main() {
  group('shell_run', () {
    ...
    test('--version', () async {
      for (final bin in [
        ...
        stdout.writeln();
        final result = (await run('$bin --version',
    ...
    test('dart compile', () async {
      const bin = 'build/native/info.exe';
  ...
  ''',);
    ...
    test('throwOnError', () async {
      const verbose = false; // devWarning(true);
      ...
      // This won't throw
      final exitCode = (await shell.run('echo --exit-code 1')).first.exitCode;
      expect(exitCode, 1);
