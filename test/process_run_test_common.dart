...
class TestSink<T> implements StreamSink<T> {
  ...
  final List<Result<T>> results = <Result<T>>[];
  ...
  final Completer _doneCompleter = Completer<dynamic>();
...
final String basicScriptExecutableExtension = Platform.isWindows ? '.bat' : '';
...
Future createEchoExecutable(String path) async {
  ...
  final String fullPath = '$path$basicScriptExecutableExtension';
  await File(fullPath)
